    ...
    <div className='navigation-bar-global w-[100vw] flex h-[56px] mb-[64px] bg-secondary justify-between fixed'>
      <Link to='/'>
        <div className='site-logo max-h-28 flex justify-between cursor-pointer'>
          <img src='../../icons/pokeball.png' className='max-h-[60px] p-2 ml-4'></img>
      ...
      <span className=' ml-auto mr-[40px] '>
        {player ? (
          <button
            ...
            className='btn-main h-[16px] p-1 m-auto mt-2'
          >
            Sign out
          </button>
        ) : (
          <Link to='/start'>
            <button className='btn-main h-[16px] p-1 text-l m-auto mt-2'> Sing in </button>
          </Link>
        )}
      </span>
      <div className='drop-down-menu relative ml-8 m-8 mt-2'>
        ...
        {dropDownMenu && (
          <div className='absolute rounded-l pl-1 pr-2 bg-emerald-400'>
            <Link to='/pokecodex'>
              <button className='text-xl rounded-l border-2 border-black bg-accent min-w-[120px] min-h-[.5rem] hover:bg-accent-light mt-1 ml-1'>
                New Game
              </button>
            </Link>
